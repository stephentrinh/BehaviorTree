...
namespace STBehaviorTree
{
    ...
    /// </summary>
    public static class BehaviorTreeNodeUtility
    {
        public static void TreeToList<T>(T root, IList<T> result) where T : BehaviorTreeNode
        {
            if (result == null)
        ...
        public static T ListToTree<T>(IList<T> list) where T : BehaviorTreeNode
        {
            // Validate input
            ...
            for (int parentIndex = 0; parentIndex < list.Count; parentIndex++)
            {
                ...
                // Fill child array
                List<BehaviorTreeNode> childList = null;
                if (childCount != 0)
                {
                    childList = new List<BehaviorTreeNode>(childCount); // Allocate once
                    childCount = 0;
                    for (int i = parentIndex + 1; i < list.Count; i++)
                    {
                        if (list[i].Depth == parentDepth + 1)
        ...
        public static void ValidateDepthValues<T>(IList<T> list) where T : BehaviorTreeNode
        {
            if (list.Count == 0)
        ...
        static bool IsChildOf<T>(T child, IList<T> elements) where T : BehaviorTreeNode
        {
            while (child != null)
            {
        ...
        public static IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements) where T : BehaviorTreeNode
        {
            if (elements.Count == 1)
